...
<template>
  <div
    ...
    :class="[
      'relative h-full',
      isDragging && 'bg-stone-100 dark:bg-neutral-700/20'
    ]"
  >
    <!-- Drop overlay -->
    <div
      v-if="isDragging"
      class="absolute inset-0 flex items-center justify-center bg-stone-100 dark:bg-neutral-700/30 bg-opacity-90 z-10 border-2 border-dashed border-stone-400 rounded-lg m-4 pointer-events-none"
    >
      <div class="text-center">
        <svg class="w-12 h-12 mx-auto text-stone-700 dark:text-stone-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
        </svg>
        <p class="text-lg font-medium text-stone-800 dark:text-stone-200">Drop files or folders here to upload</p>
      </div>
    </div>
    ...
    >
      <div
        ...
        :key="id"
        class="bg-white dark:bg-neutral-800 rounded-lg shadow-lg border dark:border-neutral-700 p-4"
      >
        <div class="flex items-center justify-between mb-1">
          <span class="text-sm font-medium truncate mr-2" :title="entry.fileName">{{ entry.fileName }}</span>
          <span class="text-sm text-stone-500 dark:text-stone-400 shrink-0">{{ entry.progress }}%</span>
        </div>
        <div class="text-xs text-stone-400 dark:text-stone-500 mb-2">
        ...
        <div class="h-2 bg-stone-200 dark:bg-neutral-600 rounded-full overflow-hidden">
          <div
            class="h-full bg-neutral-800 dark:bg-neutral-200 transition-all"
